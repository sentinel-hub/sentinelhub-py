...
class Bands:
    """ Preset collections of bands, taken from [Sentinel Hub docs](https://docs.sentinel-hub.com/api/latest/data/)
    """
    SENTINEL1_IW = (
    ...
    SENTINEL2_L1C = tuple(
        Band(name, (Unit.REFLECTANCE, Unit.DN), (np.float32, np.uint16)) for name in [
    ...
    LANDSAT_TM_L1 = (
        *(Band(f'B0{index}', (Unit.REFLECTANCE,), (np.float32,)) for index in range(1, 6)),
        Band('B06', (Unit.BRIGHTNESS_TEMPERATURE,), (np.float32,)), Band('B07', (Unit.REFLECTANCE,), (np.float32,))
    )
    LANDSAT_TM_L2 = (
        *(Band(f'B0{index}', (Unit.REFLECTANCE,), (np.float32,)) for index in range(1, 6)),
        Band('B06', (Unit.KELVIN,), (np.float32,)), Band('B07', (Unit.REFLECTANCE,), (np.float32,))
...
class MetaBands:
    ...
    LANDSAT_OT_L2 = (
        *(Band(name, (Unit.DN,), (np.uint16,)) for name in ['BQA', 'QA_RADSAT']),
        ...
        Band('ST_QA', (Unit.KELVIN,), (np.float32,)), Band('ST_TRAD', (Unit.RADIANCE,), (np.float32,)),
        Band('ST_URAD', (Unit.RADIANCE,), (np.float32,)), Band('ST_DRAD', (Unit.RADIANCE,), (np.float32,)),
        Band('ST_ATRAN', (Unit.FRACTION,), (np.float32,)), Band('ST_EMIS', (Unit.FRACTION,), (np.float32,)),
        Band('ST_EMSD', (Unit.FRACTION,), (np.float32,)), Band('ST_CDIST', (Unit.KILOMETERS,), (np.float32,)),
        Band('dataMask', (Unit.DN,), (bool,)),
    )
    LANDSAT_ETM_L1 = (
        *(Band(name, (Unit.DN,), (np.uint16,)) for name in ['BQA', 'QA_RADSAT']),
    ...
    LANDSAT_ETM_L2 = (
        *(Band(name, (Unit.DN,), (np.uint16,)) for name in ['BQA', 'QA_RADSAT']),
        Band('ST_TRAD', (Unit.RADIANCE,), (np.float32,)), Band('ST_URAD', (Unit.RADIANCE,), (np.float32,)),
        Band('ST_DRAD', (Unit.RADIANCE,), (np.float32,)), Band('ST_ATRAN', (Unit.FRACTION,), (np.float32,)),
        Band('ST_EMIS', (Unit.FRACTION,), (np.float32,)), Band('ST_EMSD', (Unit.FRACTION,), (np.float32,)),
        Band('ST_CDIST', (Unit.KILOMETERS,), (np.float32,)), Band('SR_ATMOS_OPACITY', (Unit.FRACTION,), (np.float32,)),
        Band('SR_CLOUD_QA', (Unit.DN,), (np.float32,)), Band('ST_QA', (Unit.KELVIN,), (np.float32,)),
        Band('dataMask', (Unit.DN,), (bool,)),
    )
    LANDSAT_TM_L1 = (
        *(Band(name, (Unit.DN,), (np.uint16,)) for name in ['BQA', 'QA_RADSAT']),
