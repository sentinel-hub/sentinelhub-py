...
@pytest.mark.parametrize('splitter, bbox_len', [
    [BBoxSplitter([AREA], CRS.WGS84, 5, reduce_bbox_sizes=True), 19],
    [OsmSplitter([AREA], CRS.WGS84, 15, reduce_bbox_sizes=True), 24],
    [CustomGridSplitter([AREA], CRS.WGS84, BBOX_GRID, bbox_split_shape=(3, 4), reduce_bbox_sizes=False), 41],
    [UtmGridSplitter([AREA], CRS.WGS84, bbox_size=(1200, 1200)), 16],
    [UtmZoneSplitter([AREA], CRS.WGS84, bbox_size=(1000, 1000)), 19],
    [UtmZoneSplitter([AREA], CRS.WGS84, bbox_size=(1000, 1000), offset=(500, 500)), 21],
    [
        TileSplitter([AREA], CRS.WGS84, ('2017-10-01', '2018-03-01'), tile_split_shape=40,
                     data_collection=DataCollection.SENTINEL2_L1C, reduce_bbox_sizes=True),
        13
    ],
])
def test_return_type(splitter, bbox_len):
    return_lists = [
